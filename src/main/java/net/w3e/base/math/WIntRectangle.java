...
public class WIntRectangle {
	...
	public WIntRectangle scale(double scale) {
		...
		int w = x2 - x1;
		w = BMatUtil.round((w - w * scale) / 2);
		this.x1 += w;
		this.x2 -= w;

		int h = y2 - y1;
		h = BMatUtil.round((h - h * scale) / 2);
		this.y1 += h;
		this.y2 -= h;
	...
	public final void substractTo(WIntRectangle area, WIntRectangle max) {
		this.x1 = BMatUtil.clamp(area.x1 - this.x1, max.x1, max.x2);
		this.x2 = BMatUtil.clamp(area.x2 - this.x2, max.x1, max.x2);
		this.y1 = BMatUtil.clamp(area.y1 - this.y1, max.y1, max.y2);
		this.y2 = BMatUtil.clamp(area.y2 - this.y2, max.y1, max.y2);
	}
