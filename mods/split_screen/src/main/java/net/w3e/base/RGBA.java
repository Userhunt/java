...
public class RGBA {
	...
	/*
	public RGBA multiplyAlpha(float a) {
		if (a == 1.0f) {
			return this;
		}
		return new RGBA(r, g, b, this.a * a);
	}

	public RGBA multiply(RGBA... color) {
		BufferedImage img1 = new BufferedImage(1, 1, BufferedImage.TYPE_INT_ARGB);
		img1.setRGB(0, 0, packGUI());
		...
		for (RGBA rgba : color) {
			...
			img2 = new BufferedImage(1, 1, BufferedImage.TYPE_INT_ARGB);
			img2.setRGB(0, 0, rgba.packGUI());
			img1.createGraphics().drawImage(img2, 0, 0, null);
		}

		return RGBA.unpackRGBA(img1.getRGB(0, 0));
	}*/
