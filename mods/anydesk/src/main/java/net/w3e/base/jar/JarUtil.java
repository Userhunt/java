...
public class JarUtil {
	...
	public static List<Path> getJarFolder(String folder) {
		try {
			String jarPath = JarUtil.class.getProtectionDomain().getCodeSource().getLocation().toURI().getPath();
			if (jarPath.endsWith(".jar")) {
				...
				URL resource = JarUtil.class.getClassLoader().getResource(folder);
				int source = new File(new File(new File("").getAbsolutePath()).getParentFile(), "bin/main").toString().length() + 1;
				return Files.walk(Paths.get(resource.toURI()))
					.filter(Files::isRegularFile)
					.map(path -> {
						String file = path.toString().substring(source);
