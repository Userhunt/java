...
import net.w3e.wlib.awtutils.components.JButtonGroup;
import net.w3e.wlib.awtutils.components.JImageFitPanel;
import net.w3e.wlib.awtutils.components.JImagePanel;
import net.w3e.wlib.awtutils.utils.JFrameGuiUtils;
import net.w3e.wlib.collection.identity.IdentityLinkedHashMap;

@CustomLog
public class BezierFrame extends JFrame implements MouseListener, MouseMotionListener, JFrameGuiUtils {
	...
	public BezierFrame() {
		...
		this.buttons.getFirst().setSelected(true);
		...
		for (JRadioButton button : this.buttons) {
			button.addChangeListener(_ -> this.update(false));
			group.add(button);
			radio.add(button);
		}
		...
		this.image = new JImageFitPanel(image, SIZE * SCALE, SIZE * SCALE, false);
	...
	private Vec2I transform(Vec2 vector) {
	...
	private void update(boolean force) {
		...
		Float2ObjectFunction<Vec2D> function;
		if (i == 0) {
			function = (t) -> this.first.lerp(this.second, t);
		} else if (i == 1) {
			function = (t) -> BezierCurve.curve1(t, this.first, this.points.getFirst(), this.second);
		} else if (i == 2) {
			function = (t) -> BezierCurve.curve2(t, this.first, this.points.getFirst(), this.points.get(1), this.second);
		} else {
			function = _ -> Vec2D.ZERO;
		}
	...
	private void paintPoint(Graphics2D g, Vec2I point, String text) {
		try {
	...
	public final void mousePressed(MouseEvent e) {
		...
		if (!this.points.isEmpty()) {
			map.put(this.points.getFirst(), SelectedPoint.LIST_0);
		}
	...
	private boolean isAround(Vec2 point, Vec2I mouse) {
