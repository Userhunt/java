...
public class NoiseFrame extends JFrame implements ChangeListener, JFrameGuiUtils {
	...
	private JSlider initSlider(int value) {
	...
	private void initRadioButton(Mode mode) {
		JRadioButton radio = new JButtonGroup.JRadioButton<>(mode.name().substring(0, 1).toUpperCase() + mode.name().substring(1), mode);
