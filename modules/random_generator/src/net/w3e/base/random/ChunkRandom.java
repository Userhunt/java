...
public class ChunkRandom extends CheckedRandom {
	private final IRandom baseRandom;
	private int sampleCount;

	public ChunkRandom(IRandom baseRandom) {
		super(0L);
	...
	public int next(int bits) {
		++this.sampleCount;
		IRandom random = this.baseRandom;
	...
	public static IRandom getSlimeRandom(int chunkX, int chunkZ, long worldSeed, long scrambler) {
		return new CheckedRandom(worldSeed + (long)(chunkX * chunkX * 4987142) + (long)(chunkX * 5947611) + (long)(chunkZ * chunkZ) * 4392871L + (long)(chunkZ * 389711) ^ scrambler);
	}
	...
	public static enum RandomProvider {
		...
		private final LongFunction<IRandom> provider;

		private RandomProvider(LongFunction<IRandom> provider) {
